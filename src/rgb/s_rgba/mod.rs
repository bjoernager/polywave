...
///
/// This type is equivalent to [`SRgb`] with an added alpha component.
#[repr(transparent)]
...
pub struct SRgba<T>(Rgba<T>);

impl<T: Component> SRgba<T> {
	/// Constructs a new sRGBA colour.
	#[inline(always)]
	#[must_use]
...
impl<T: Component> Colour for SRgba<T> { }

impl<T: Component> DefinedGamut for SRgba<T> { }
